...
def process_seq(artiq_backend.ArtiqBackend ab):
    ab.process_seq()
...
class ChannelsInfo:
    def __init__(self, artiq_backend.ChannelsInfo info):
        self.urukul_busses = [new_urukul_bus(bus) for bus in info.urukul_busses]
        self.ttlchns = [new_ttl_channel(ttl) for ttl in info.ttlchns]
...
def get_channel_info(artiq_backend.ArtiqBackend ab):
    return ChannelsInfo(ab.channels)
