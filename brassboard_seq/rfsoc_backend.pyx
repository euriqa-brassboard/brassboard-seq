...
cdef _Jaqal_v1_3.ModTypeMask get_modtype_mask_v1_3(modtype):
    cdef int modmask = 0
    if isinstance(modtype, int):
        ...
        for t in modtype:
            ...
            modmask |= PyLong_AsLong(<object>maskp)
    return <_Jaqal_v1_3.ModTypeMask>modmask

cdef uint8_t get_chn_mask_v1_3(channels):
    cdef int chnmask = 0
...
cdef class Jaqal_v1_3:
    ...
    def apply_modtype_mask(JaqalInst_v1_3 pulse, modtype, /):
        modmask = get_modtype_mask_v1_3(modtype)
    ...
    def program_SLUT(channels, _saddrs, _paddrs, _modtypes, /):
        ...
        for i in range(n):
            saddrs[i] = _saddrs[i]
            modtypes[i] = get_modtype_mask_v1_3(_modtypes[i])
            paddrs[i] = _paddrs[i]
