...
static void print_generic(py_stringio &io, PyObject *obj, int indent, int cur_indent);

static inline bool needs_quote(PyObject *s)
{
    // Good enough for now........
    auto len = PyUnicode_GET_LENGTH(s);
    for (auto [i, c]: pystr_iter(s)) {
        if (c == ' ' && (i == 0 || i == len - 1))
            return true;
        if (c == '"' || c == ':' || c == '\n' || c == '\b' || c == '\\') {
            return true;
...
static inline void print_string(py_stringio &io, PyObject *s, int indent, int cur_indent)
{
...
static inline void print_single_field_dict(py_stringio &io, PyObject *obj, int indent,
...
static inline void print_dict_field(py_stringio &io, PyObject *k, PyObject *v, int indent)
{
...
static inline void print_dict(py_stringio &io, PyObject *obj, int indent, int cur_indent)
{
...
static inline void print_scalar(py_stringio &io, PyObject *obj, int indent, int cur_indent)
{
    if (is_bool_obj(obj))
...
static void print_array_str(py_stringio &io, std::vector<py_object> &strary,
                            bool all_short_scalar, int indent, int cur_indent)
{
    if (all_short_scalar) {
        ...
        auto print_list_until = [&] (size_t i) {
            ...
            for (; print_idx < i; print_idx++) {
                if (!isfirst)
                    io.write_ascii(", ");
                isfirst = false;
                io.write((PyObject*)strary[print_idx]);
            }
        };
        for (size_t i = 0; i < nele; i++) {
            ...
            linelen += PyUnicode_GET_LENGTH((PyObject*)strary[i]) + 2;
        }
        ensure_prefix();
...
static inline void print_array_iter(py_stringio &io, auto &&iter,
                                    int indent, int cur_indent)
{
    std::vector<py_object> strary;
    bool all_short_scalar = true;
    for (auto [i, v]: iter) {
        if (is_bool_obj(v)) {
            strary.emplace_back(py_newref(get_value_bool(v, -1) ?
                                          "true"_py : "false"_py));
        }
            ...
            strary.emplace_back(io2.getvalue());
            all_short_scalar = false;
        }
    }
...
static inline void print_array_numpy(py_stringio &io, PyArrayObject *ary,
                                     int indent, int cur_indent)
{
    std::vector<py_object> strary;
...
{
    if (PyList_Check(obj))
        return print_array_iter(io, pylist_iter(obj), indent, cur_indent);
    if (PyTuple_Check(obj))
        return print_array_iter(io, pytuple_iter(obj), indent, cur_indent);
...
void print(py_stringio &io, PyObject *obj, int indent)
{
