...
struct ChannelGen {
    struct Gate {
        int nrep;
        std::vector<std::pair<int8_t,int>> pulse_ids;
    };
    struct Pulse {
        int64_t time;
        int64_t len: 40;
    ...
    {
        int64_t len = (inst >> 40 * 4)[0] & ((int64_t(1) << 40) - 1);
        block.pulse_ids.push_back({cycle, len, (int8_t)param, pulses.get_addr(inst)});
    }
