...
static inline void populate_bseq_values(auto comp, CompiledBasicSeq &cbseq)
{
    ...
    for (auto next_id: pyx_fld(bseq, next_bseq)) {
        ...
        comp->cseq.basic_seqs.resize(nbcseqs + 1);
        // Get the reference again after resizing
        next_cbseq = &comp->cseq.basic_seqs[next_id];
        auto &new_cbseq = comp->cseq.basic_seqs.back();
                                             ...
                                             _RampFunctionBase*, Backend*)
{
    ...
    for (auto [i, bseq]: pylist_iter<BasicSeq>(pyx_fld(seq, basic_seqs))) {
        ...
        cseq.basic_seqs[i].total_time =
            time_mgr->__pyx_vtab->compute_all_times(time_mgr, age, pyage);
    }
    ...
    for (auto [name, backend]: pydict_iter<Backend>(comp->backends)) {
        throw_if(backend->__pyx_vtab->runtime_finalize(backend, cseq, age, pyage) < 0);
    }
}
